...
class SBO_points_list_reader(object):
	...
	def read_points(self, sheetname):
		...
		for n, point in points_df.iterrows():
			...
			# print(not isinstance(point['type'], str))
			# print(point['type'])
			if str(point['type']).lower() not in ['empty', 'nan', '']:
				points_list.append(self.read_point(point))
		return points_list

	def read_point(self, point):
		"""
		read excel worksheet row into a point dict object

		TODO:
		need to santise the ch# field.
		ch# field may be:
		1. in a numeric channel number, eg 1
		2. a IO type-channel number combo (ie mixed IO) eg DO-1, UI-1

		for the second kind, need to convert to just channel number for SBO
		...
		ch_num_str = self.get_channel_number(str(point["ch#"]))
		if "output" in point["type"].lower():
			point_dict["properties"].append(
				{
					"OutputChannelNumber": ch_num_str
				}
			)
		elif "input" in point["type"].lower():
			point_dict["properties"].append(
				{
					"InputChannelNumber": ch_num_str
				}
			)
		# set label property
		point_dict["properties"].append(
	...
	def get_channel_number(self, ch_num_raw):
		"""
		pass in the raw point channel number field as a string from the Excel worksheet
		return the excplicit channel number as a string.
	...
	def reduce_str(self, phrase, lower=False):
		...
		TODO:
		turn the replace().replace().replace()... to a function with a list of invalid characters

		"""
		print(phrase, type(phrase))
		reduced = phrase.replace(" ", "").replace("-", "").replace("_", "").replace(":", "").replace(".", "")
		if lower:
...
if __name__ == '__main__':
	...
	xlfile = '2907309Q B04-Chillers-AS040 Points List R0.5.xlsx'
